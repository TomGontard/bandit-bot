...
export async function pickWinners(guild, amount) {
  ...
  const linkedDocs = await UserLink.find(
    { discordId: { $in: eligible.map((e) => e.id) } },
    'discordId'
  );
  ...
  const winners = [];
  let triesSafe = 10000; // avoid infinite loop if amount > pool
...
export async function grantWhitelist(discordId, staffId) {
